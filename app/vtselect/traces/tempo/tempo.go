...
var (
	tempoDefaultSearchWindow = flag.Duration("search.tempo.defaultSearchWindow", time.Hour, "The window of searching for traces if start or end time is not provided."+
		"It affects both Tempo's /api/search API.")
)
...
func processQueryV2Request(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	w.Write(b.Bytes())
	f, err := os.OpenFile("./tmp_tempo_resp.bin", os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0666)
	if err != nil {
		panic(err)
	}
	f.Write(b.Bytes())
